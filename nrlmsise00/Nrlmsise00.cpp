...
#include <nrlmsise00/data.cpp>
#include <nrlmsise00/math.hpp>
...
void Nrlmsise00::glatf(const double& lat, double& gv, double& reff)
{
...
double Nrlmsise00::ccor(const double& alt, const double& r, const double h1, const double zh)
{
    double e = (alt - zh) / h1;
    if (e > 70.0) { return exp(0); }
    if (e < -70.0) { return exp(r); }
    return exp(r / (1.0 + exp(e)));
}

double Nrlmsise00::ccor2(const double& alt, const double& r, const double h1, const double zh, const double h2)
{
    double e1 = (alt - zh) / h1;
    double e2 = (alt - zh) / h2;
    if ((e1 > 70.0) or (e2 > 70.0)) { return exp(0); }
    if ((e1 < -70.0) and (e2 < -70.0)) { return exp(r); }
    return exp(r / (1.0 + 0.5 * (exp(e1) + exp(e2))));
}

double Nrlmsise00::scalh(const double& alt, const double xm, const double temp) const
{
    double g = d_gsurf / (pow((1.0 + alt / d_re), 2.0));
    return data::d_RGAS * temp / (g * xm);
}

double Nrlmsise00::dnet(double& dd, const double& dm, const double& zhm, const double& xmm, const double xm) const
{
    double a;
    double ylog;
    a = zhm / (xmm - xm);
...
inline double Nrlmsise00::zeta(const double& zz, double zl) const { return ((zz - zl) * (d_re + zl) / (d_re + zz)); }

double Nrlmsise00::densm(
    const double& alt,
    const double& d0,
    const double xm,
    double* tz,
...
// Eq. A24c
static inline double sumex(const double& ex) { return (1.0 + (1.0 - pow(ex, 19.0)) / (1.0 - ex) * pow(ex, 0.5)); }
...
{
    return (g0(ap[1], p) + (g0(ap[2], p) * ex + g0(ap[3], p) * ex * ex + g0(ap[4], p) * pow(ex, 3.0) +
                            (g0(ap[5], p) * pow(ex, 4.0) + g0(ap[6], p) * pow(ex, 12.0)) * (1.0 - pow(ex, 8.0)) / (1.0 - ex))) /
           sumex(ex);
}

double Nrlmsise00::globe7(
    const std::array<double, 150>& p,
    const int doy,
    const double sec,
    const double& g_lat,
    const double& g_long,
...
)
{
    ...
    /*  TIME INDEPENDENT */
    t[1] = (p[1] * a_plg[0][2] + p[2] * a_plg[0][4] + p[22] * a_plg[0][6]) + (p[14] * a_plg[0][2]) * d_dfa * a_swc[1] +
           p[26] * a_plg[0][1];
    ...
    if (a_sw[7]) {
        ...
        t[6] = f2 * ((p[3] * a_plg[1][1] + p[4] * a_plg[1][3] + p[27] * a_plg[1][5] + t71) * d_ctloc +
                     (p[6] * a_plg[1][1] + p[7] * a_plg[1][3] + p[28] * a_plg[1][5] + t72) * d_stloc);
    }
    ...
    if (a_sw[8]) {
        ...
        t[7] = f2 * ((p[5] * a_plg[2][2] + p[41] * a_plg[2][4] + t81) * d_c2tloc +
                     (p[8] * a_plg[2][2] + p[42] * a_plg[2][4] + t82) * d_s2tloc);
    }

    /* TERDIURNAL */
    if (a_sw[14]) {
        t[13] = f2 * ((p[39] * a_plg[3][3] + (p[93] * a_plg[3][4] + p[46] * a_plg[3][6]) * cd14 * a_swc[5]) * d_s3tloc +
                      (p[40] * a_plg[3][3] + (p[94] * a_plg[3][4] + p[48] * a_plg[3][6]) * cd14 * a_swc[5]) * d_c3tloc);
    }
    ...
    if ((a_sw[10]) && (g_long > -1000.0)) {
        ...
        if (a_sw[11]) {
            t[10] = (1.0 + p[80] * d_dfa * a_swc[1]) *
                    ((p[64] * a_plg[1][2] + p[65] * a_plg[1][4] + p[66] * a_plg[1][6] + p[103] * a_plg[1][1] +
                      p[104] * a_plg[1][3] + p[105] * a_plg[1][5] +
                      a_swc[5] * (p[109] * a_plg[1][1] + p[110] * a_plg[1][3] + p[111] * a_plg[1][5]) * cd14) *
                         cos(data::d_DGTR * g_long) +
                     (p[90] * a_plg[1][2] + p[91] * a_plg[1][4] + p[92] * a_plg[1][6] + p[106] * a_plg[1][1] +
                      p[107] * a_plg[1][3] + p[108] * a_plg[1][5] +
        ...
        if (a_sw[12]) {
            t[11] = (1.0 + p[95] * a_plg[0][1]) * (1.0 + p[81] * d_dfa * a_swc[1]) *
                    ...
                    ((p[68] * a_plg[0][1] + p[69] * a_plg[0][3] + p[70] * a_plg[0][5]) * cos(data::d_SR * (sec - p[71])));
            t[11] += a_swc[11] * (p[76] * a_plg[2][3] + p[77] * a_plg[2][5] + p[78] * a_plg[2][7]) *
        ...
        if (a_sw[13]) {
            if (a_sw[9] == -1) {
                if (p[51]) {
                    t[12] = a_apt[0] * a_swc[11] * (1. + p[132] * a_plg[0][1]) *
                            ...
                            a_apt[0] * a_swc[11] * a_swc[5] *
                                (p[133] * a_plg[1][1] + p[134] * a_plg[1][3] + p[135] * a_plg[1][5]) * cd14 *
                                cos(data::d_DGTR * (g_long - p[136])) +
                            a_apt[0] * a_swc[12] * (p[55] * a_plg[0][1] + p[56] * a_plg[0][3] + p[57] * a_plg[0][5]) *
                ...
                t[12] = d_apdf * a_swc[11] * (1.0 + p[120] * a_plg[0][1]) *
                        ...
                        d_apdf * a_swc[11] * a_swc[5] * (p[115] * a_plg[1][1] + p[116] * a_plg[1][3] + p[117] * a_plg[1][5]) *
                            cd14 * cos(data::d_DGTR * (g_long - p[118])) +
                        d_apdf * a_swc[12] * (p[83] * a_plg[0][1] + p[84] * a_plg[0][3] + p[85] * a_plg[0][5]) *
...
double Nrlmsise00::glob7s(const std::array<double, 100>& p, const int doy, const double& g_long)
{
    double t[14];
    double tt;
    double cd32, cd18, cd14, cd39;
    ...
    /* time independent */
    t[1] = p[1] * a_plg[0][2] + p[2] * a_plg[0][4] + p[22] * a_plg[0][6] + p[26] * a_plg[0][1] + p[14] * a_plg[0][3] +
           p[59] * a_plg[0][5];
    ...
    /* TERDIURNAL */
    if (a_sw[14]) { t[13] = p[39] * a_plg[3][3] * d_s3tloc + p[40] * a_plg[3][3] * d_c3tloc; }

    /* MAGNETIC ACTIVITY */
    if (a_sw[9]) {
        if (a_sw[9] == 1) { t[8] = d_apdf * (p[32] + p[45] * a_plg[0][2] * a_swc[2]); }
        if (a_sw[9] == -1) { t[8] = (p[50] * a_apt[0] + p[96] * a_plg[0][2] * a_apt[0] * a_swc[2]); }
    }

    /* LONGITUDINAL */
    if (!((a_sw[10] == 0) || (a_sw[11] == 0) || (g_long <= -1000.0))) {
        t[10] = (1.0 +
                 a_plg[0][1] * (p[80] * a_swc[5] * cos(data::d_DR * (doy - p[81])) +
                                p[85] * a_swc[6] * cos(2.0 * data::d_DR * (doy - p[86]))) +
                 p[83] * a_swc[3] * cos(data::d_DR * (doy - p[84])) + p[87] * a_swc[4] * cos(2.0 * data::d_DR * (doy - p[88]))) *
                ((p[64] * a_plg[1][2] + p[65] * a_plg[1][4] + p[66] * a_plg[1][6] + p[74] * a_plg[1][1] +
                  p[75] * a_plg[1][3] + p[76] * a_plg[1][5]) *
                     cos(data::d_DGTR * g_long) +
                 (p[90] * a_plg[1][2] + p[91] * a_plg[1][4] + p[92] * a_plg[1][6] + p[77] * a_plg[1][1] +
                  p[78] * a_plg[1][3] + p[79] * a_plg[1][5]) *
                     sin(data::d_DGTR * g_long));
    }
    tt = 0;
    for (i = 0; i < 14; i++) {
...
)
{
    ...
    if (alt < 300.0) {
        ...
        a_meso_tgn1[1] = data::ptm[8] * data::pma[8][0] * (1.0 + a_sw[18] * a_sw[20] * glob7s(data::pma[8], doy, g_long)) *
                         a_meso_tn1[4] * a_meso_tn1[4] / (pow((data::ptm[4] * data::ptl[3][0]), 2.0));
    }
    else {
        ...
        a_meso_tgn1[1] =
            data::ptm[8] * data::pma[8][0] * a_meso_tn1[4] * a_meso_tn1[4] / (pow((data::ptm[4] * data::ptl[3][0]), 2.0));
    }
    ...
    zhf = data::pdl[1][24] *
          (1.0 + a_sw[5] * data::pdl[0][24] * sin(data::d_DGTR * g_lat) * cos(data::d_DR * (doy - data::pt[13])));
    t[0] = tinf;
    xmm  = data::pdm[2][4];
    z    = alt;
    ...
    d[2] = densu(
        z, db28, tinf, tlb, 28.0, alpha[2], &t[1], data::ptm[5], s, mn1, zn1.data(), a_meso_tn1.data(), a_meso_tgn1.data()
    );
    dd = d[2];
    /* Turbopause */
    ...
    if ((a_sw[15]) && (z <= altl[2])) {
        ...
        d_dm28 = densu(
            z, b28, tinf, tlb, xmm, alpha[2], &tz, data::ptm[5], s, mn1, zn1.data(), a_meso_tn1.data(), a_meso_tgn1.data()
        );
        /*  Net density at Alt */
    ...
    /*  Diffusive density at Alt */
    d[0] =
        densu(z, db04, tinf, tlb, 4., alpha[0], &t[1], data::ptm[5], s, mn1, zn1.data(), a_meso_tn1.data(), a_meso_tgn1.data());
    dd = d[0];
    if ((a_sw[15]) && (z < altl[0])) {
        /*  Turbopause */
        zh04 = data::pdm[0][2];
        ...
        /*  Mixed density at Alt */
        d_dm04 =
            densu(z, b04, tinf, tlb, xmm, 0.0, &t[1], data::ptm[5], s, mn1, zn1.data(), a_meso_tn1.data(), a_meso_tgn1.data());
        zhm04 = zhm28;
        /*  Net density at Alt */
    ...
    /*   Diffusive density at Alt */
    d[1] =
        densu(z, db16, tinf, tlb, 16., alpha[1], &t[1], data::ptm[5], s, mn1, zn1.data(), a_meso_tn1.data(), a_meso_tgn1.data());
    dd = d[1];
    if ((a_sw[15]) && (z <= altl[1])) {
        /*   Turbopause */
        zh16 = data::pdm[1][2];
        ...
        /*  Mixed density at Alt */
        d_dm16 =
            densu(z, b16, tinf, tlb, xmm, 0.0, &t[1], data::ptm[5], s, mn1, zn1.data(), a_meso_tn1.data(), a_meso_tgn1.data());
        zhm16 = zhm28;
        /*  Net density at Alt */
    ...
    /*   Diffusive density at Alt */
    d[3] =
        densu(z, db32, tinf, tlb, 32., alpha[3], &t[1], data::ptm[5], s, mn1, zn1.data(), a_meso_tn1.data(), a_meso_tgn1.data());
    dd = d[3];
    if (a_sw[15]) {
        if (z <= altl[3]) {
            /* Turbopause */
            ...
            /* Mixed density at Alt */
            d_dm32 = densu(
                z, b32, tinf, tlb, xmm, 0., &t[1], data::ptm[5], s, mn1, zn1.data(), a_meso_tn1.data(), a_meso_tgn1.data()
            );
            zhm32 = zhm28;
            /* Net density at Alt */
    ...
    /*   Diffusive density at Alt */
    d[4] =
        densu(z, db40, tinf, tlb, 40., alpha[4], &t[1], data::ptm[5], s, mn1, zn1.data(), a_meso_tn1.data(), a_meso_tgn1.data());
    dd = d[4];
    if ((a_sw[15]) && (z <= altl[4])) {
        /*   Turbopause */
        zh40 = data::pdm[4][2];
        ...
        /*  Mixed density at Alt */
        d_dm40 =
            densu(z, b40, tinf, tlb, xmm, 0.0, &t[1], data::ptm[5], s, mn1, zn1.data(), a_meso_tn1.data(), a_meso_tgn1.data());
        zhm40 = zhm28;
        /*  Net density at Alt */
    ...
    /*   Diffusive density at Alt */
    d[6] =
        densu(z, db01, tinf, tlb, 1., alpha[6], &t[1], data::ptm[5], s, mn1, zn1.data(), a_meso_tn1.data(), a_meso_tgn1.data());
    dd = d[6];
    if ((a_sw[15]) && (z <= altl[6])) {
        /*   Turbopause */
        zh01 = data::pdm[5][2];
        ...
        /*  Mixed density at Alt */
        d_dm01 =
            densu(z, b01, tinf, tlb, xmm, 0., &t[1], data::ptm[5], s, mn1, zn1.data(), a_meso_tn1.data(), a_meso_tgn1.data());
        zhm01 = zhm28;
        /*  Net density at Alt */
    ...
    /*   Diffusive density at Alt */
    d[7] = densu(
        z, db14, tinf, tlb, 14.0, alpha[7], &t[1], data::ptm[5], s, mn1, zn1.data(), a_meso_tn1.data(), a_meso_tgn1.data()
    );
    dd = d[7];
    if ((a_sw[15]) && (z <= altl[7])) {
        /*   Turbopause */
        ...
        /*  Mixed density at Alt */
        d_dm14 =
            densu(z, b14, tinf, tlb, xmm, 0.0, &t[1], data::ptm[5], s, mn1, zn1.data(), a_meso_tn1.data(), a_meso_tgn1.data());
        zhm14 = zhm28;
        /*  Net density at Alt */
    ...
    dd =
        densu(z, db16h, tho, tho, 16., alpha[8], &t[1], data::ptm[5], s, mn1, zn1.data(), a_meso_tn1.data(), a_meso_tgn1.data());
    zsht = data::pdm[7][5];
    zmho = data::pdm[7][4];
...
)
{
    ...
    a_meso_tgn2[1] = data::pavgm[8] * data::pma[9][0] * (1.0 + a_sw[20] * a_sw[22] * glob7s(data::pma[9], doy, g_long)) *
                     a_meso_tn2[3] * a_meso_tn2[3] / (pow((data::pma[2][0] * data::pavgm[2]), 2.0));
    ...
    if (alt <= zn3[0]) {
        ...
        a_meso_tgn3[1] = data::pma[7][0] * data::pavgm[7] * (1.0 + a_sw[22] * glob7s(data::pma[7], doy, g_long)) *
                         a_meso_tn3[4] * a_meso_tn3[4] / (pow((data::pma[6][0] * data::pavgm[6]), 2.0));
    }
